...
int fill_buffer_to(uint32_t *buf)
{
#if BUFFER_MODE == 0
    buffer_idx = 0;
    ...
    /* Configure destination and submit */
    dmac_write(AXI_DMAC_REG_DEST_ADDRESS, (uint32_t)(uintptr_t)buf);
    dmac_write(AXI_DMAC_REG_X_LENGTH,     DMA_TRANSFER_BYTES - 1);
    dmac_write(AXI_DMAC_REG_TRANSFER_SUBMIT, AXI_DMAC_TRANSFER_SUBMIT);
    ...
    timeout_cnt = 0;
    while (!(dmac_read(AXI_DMAC_REG_TRANSFER_DONE) & (1u << transfer_id))) {
        /* Spin - DMA completes in ~28 us */
    }

    /* Invalidate cache so CPU reads DMA-written data from DDR */
    Xil_DCacheInvalidateRange((INTPTR)buf, DMA_TRANSFER_BYTES);
